...
static const SDL_VideoInfo* g_gfx_VideoInfo = NULL;
...
void gfx_init(const char* caption)
{
	SDL_Init(SDL_INIT_VIDEO);	
	g_gfx_VideoInfo = SDL_GetVideoInfo();
	
	/*assert( (g_gfx_screen = SDL_SetVideoMode(g_gfx_VideoInfo->current_w, g_gfx_VideoInfo->current_h - 50, 0, SDL_HWSURFACE|SDL_DOUBLEBUF)) != NULL );*/
	assert( (g_gfx_screen = SDL_SetVideoMode(1000, 720, 0, SDL_HWSURFACE|SDL_DOUBLEBUF)) != NULL );
	g_gfx_window_width = g_gfx_VideoInfo->current_w;
	g_gfx_window_height = g_gfx_VideoInfo->current_h - 50;
